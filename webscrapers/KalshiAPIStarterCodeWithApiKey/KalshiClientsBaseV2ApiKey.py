...
class ExchangeClient(KalshiClient):
    ...
    def get_candlesticks(
        ...
        """Gets historical candlesticks for a market.

        Args:
            series_ticker: Unique identifier for the series (e.g. 'PRES')
            ticker: Unique identifier for the market
            start_ts: Start timestamp (Unix timestamp)
            end_ts: End timestamp (Unix timestamp)
            period_interval: Length of each candlestick period in minutes (1, 60, or 1440)

        Returns:
            Dictionary containing candlesticks data
        """
        # Validate period_interval is valid (1 min, 1 hour, or 1 day)
        valid_intervals = {1, 60, 1440}  # minutes in a day = 1440
        if period_interval not in valid_intervals:
            raise ValueError(f"period_interval must be one of {valid_intervals}")

        # Validate end_ts is within 5000 periods of start_ts
        max_duration = period_interval * 5000 * 60  # Convert to seconds
        if end_ts - start_ts > max_duration:
            raise ValueError(f"end_ts must be within {5000} periods of start_ts")

        # Use the series endpoint pattern from v2 API
        candlesticks_url = (
            f"{self.series_url}/{series_ticker}/markets/{ticker}/candlesticks"
        )
        params = {
            "start_ts": start_ts,
            "end_ts": end_ts,
        ...
        dictr = self.get(candlesticks_url + query_string)
        return dictr
